...
To run the app execute ./mvnw spring-boot:run in the project root folder.

Once the server loads it should display something like 
Started LimegramApplication in 1.392 seconds (JVM running for 1.668)

You can then access the application in a browser at localhost:8080/index.html

An image is always associated with a user, images are uploaded with a rest request to: http://localhost:8080/upload/{userId}
